...
import edu.aku.hassannaqvi.matiari_cohorts.core.MainApp.forms
...
class EndingActivity : AppCompatActivity() {
    ...
    private fun saveDraft() {
        ...
        forms.istatus = statusValue
        forms.istatus96x = if (bi.a0696x.text.toString().trim().isEmpty()) "-1" else bi.a0696x.text.toString()
        forms.endingdatetime = SimpleDateFormat("dd-MM-yyyy HH:mm").format(Date().time)
        /*MainApp.formsSF.istatus = statusValue
        MainApp.formsSF.istatus96x = bi.a0696x.text.toString()
        MainApp.formsSF.endingdatetimewq32    `1 = SimpleDateFormat("dd-MM-yy HH:mm").format(Date().time)*/
    }
